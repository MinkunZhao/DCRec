...
class SGCF_eg_crv(BaseModel):
    def __init__(self, data_handler):
        super(SGCF_eg_crv, self).__init__(data_handler)
        # 原始协同图参数
        ...
        # 协同通道embedding - SGCF特点：只需要基础的embedding
    ...
    def _propagate_co(self, adj, embeds):
        """SGCF协同通道传播 - 简化的图卷积
        核心思想：简化GCN操作，仅保留消息传递，移除变换矩阵和非线性激活
        """
        all_embeds = [embeds]
