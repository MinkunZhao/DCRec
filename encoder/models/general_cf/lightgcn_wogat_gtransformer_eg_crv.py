...
from torch_geometric.nn import TransformerConv  # 修改：使用 TransformerConv 替代 GATConv
...
"""
    ...
    def batch_process_nodes(self, adj, batch_size=10, max_nodes=1000, save_interval=100):
        """批量处理节点获取LLM建议的门控权重"""
        # 处理的节点数量上限
...
class LightGCN_wogat_gtransformer_eg_crv(BaseModel):
    """
    消融实验版本：语义侧由图注意力网络 (GAT) 改为图Transformer（使用 TransformerConv）
    目的是对比验证原模型中GAT模块的有效性，即证明GAT比图Transformer获得更优结果
    """
    def __init__(self, data_handler):
        super(LightGCN_wogat_gtransformer_eg_crv, self).__init__(data_handler)
        # 原始协同图参数
        ...
        # 消融实验核心变化：使用图Transformer模块（TransformerConv）替换原有的 GAT 模块
        self.transformer_layers = nn.ModuleList([
            TransformerConv(
                in_channels=self.embedding_size,
                out_channels=self.embedding_size,
                heads=self.hyper_config['gat_heads'],  # 复用相同超参数名称
                concat=False,
                dropout=0.1,      # 可根据实际需要调整 dropout 参数
                beta=True,        # 开启 beta 参数，允许网络学习注意力分配
                edge_dim=1
            )
    ...
    def _propagate_sem(self, embeds):
        ...
        # 修改：将边属性从形状 [E] 转换为 [E, 1]
        edge_weight = self.semantic_adj.values().unsqueeze(-1)
        for conv in self.transformer_layers:
            embeds = conv(embeds, edge_index, edge_attr=edge_weight)
    ...
    def cal_loss(self, batch_data):
        ...
        # 语义图前向传播（keep_rate 固定为1.0不做edge drop）
        ...
        reg_loss = self.reg_weight * (
            ...
            sum(t.norm(p) for p in self.transformer_layers.parameters())
        )
        ...
        gate_loss /= 3.0
        gate_loss *= self.gate_supervision_weight
