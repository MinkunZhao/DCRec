...
from torch_geometric.nn import SAGEConv  # 替换GATConv为SAGEConv
...
class LightGCN_wogat_GraphSage_eg_crv(BaseModel):  # 修改类名以区分
    def __init__(self, data_handler):
        super(LightGCN_wogat_GraphSage_eg_crv, self).__init__(data_handler)
        # 原始协同图参数
    ...
    def _init_weights(self):
        ...
        for sage in self.sage_layers:
            # 初始化线性变换参数
            if hasattr(sage, 'lin_l'):
                init(sage.lin_l.weight)
            if hasattr(sage, 'lin_r'):
                init(sage.lin_r.weight)
    ...
    def _propagate_sem(self, embeds):
        ...
        # GraphSage不需要edge_weight参数
        for sage in self.sage_layers:
            embeds = sage(embeds, edge_index)
    ...
    def cal_loss(self, batch_data):
        ...
        reg_loss = self.reg_weight * (
                ...
                sum(t.norm(p) for p in self.sage_layers.parameters())  # 修改为sage_layers
        )
