...
class LightGCN_wogate_crv(BaseModel):
    def __init__(self, data_handler):
        super(LightGCN_wogate_crv, self).__init__(data_handler)
        # 原始协同图参数
        ...
        # 消融实验：使用直接拼接的方式替代门控融合
        # 添加一个投影层，将拼接后的双倍维度转回原始维度
        self.projection = nn.Linear(2 * self.embedding_size, self.embedding_size)

        # 新增：LLM指导管理器（虽然不再使用门控机制，但保留以保证代码结构一致）
    ...
    def forward(self, keep_rate=1.0):
        ...
        # 消融实验：直接拼接双通道embedding并投影
        concat_embeds = t.cat([co_embeds, sem_embeds], dim=1)  # 拼接
        fused_embeds = self.projection(concat_embeds)  # 投影回原始维度

        # 返回一个None作为gate，以保持与原始模型接口兼容
        return fused_embeds[:self.user_num], fused_embeds[self.user_num:], None
    ...
    def cal_loss(self, batch_data):
        ...
        # 结构图前向传播
        user_embeds_struct, item_embeds_struct, _ = self.forward(self.keep_rate)
        # 语义图前向传播
        user_embeds_sem, item_embeds_sem, _ = self.forward(1.0)
        ...
        user_embeds, item_embeds, _ = self.forward(self.keep_rate)
        ...
        reg_loss = self.reg_weight * (
                ...
                t.norm(self.projection.weight)  # 添加投影层的正则化
        )
