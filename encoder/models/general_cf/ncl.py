...
class NCL(LightGCN):
	...
	def cal_loss(self, batch_data):
		...
		# 动态生成 kmeans_flags（示例）
		kmeans_flags = t.zeros_like(ancs)  # 根据需求调整逻辑
		if hasattr(self, 'user2cluster') == False:
			self._cluster()
		embeds, embeds_list = self.forward(self.adj, self.keep_rate)
	...
	def full_predict(self, batch_data):
		embeds, _ = self.forward(self.adj, 1.0)
