...
class LightGCN_wogat_eg_crv(BaseModel):  # 更改类名以区分消融实验模型
    def __init__(self, data_handler):
        super(LightGCN_wogat_eg_crv, self).__init__(data_handler)
        # 原始协同图参数
    ...
    def _propagate_sem(self, embeds):
        ...
        for gcn in self.gcn_layers:
            # GCN输入：节点特征和边索引，以及可选的边权重
            embeds = gcn(embeds, edge_index, edge_weight)
    ...
    def cal_loss(self, batch_data):
        ...
        reg_loss = self.reg_weight * (
                ...
                sum(t.norm(p) for p in self.gcn_layers.parameters())
        )
